...
export async function GET(req: NextRequest, { params }: { params: Promise<{ aid: string }> }) {
  ...
  if (attachment.task_id) {
    allowed = !!(await getProjectMembership(attachment.project_id, auth.agent.id));
  }
  ...
  if (!allowed) {
    return NextResponse.json({ error: 'Forbidden', code: 'FORBIDDEN' } satisfies ApiError, { status: 403 });
  }
